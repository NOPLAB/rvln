#!/usr/bin/env bash
# scripts/vla.sh — thin driver for the rvln Docker stacks.
#
...
# RVLN_JETSON=1 (or =0 to disable, e.g. cross-build on an aarch64 host).
is_jetson() {
    case "${RVLN_JETSON:-}" in
        1) return 0 ;;
        0) return 1 ;;
...
declare -A IMAGES=(
    [asyncvla]="rvln-asyncvla"
    [omnivla]="rvln-omnivla"
    ...
    [movla]="rvln-movla"
    [asyncvla-jetson]="rvln-asyncvla-jetson"
    [omnivla-jetson]="rvln-omnivla-jetson"
    [omnivla_edge-jetson]="rvln-omnivla-jetson"
    [test]="rvln-test"
    [real]="rvln-real"
    [sim]="rvln-sim"
)
...
Jetson AGX Orin (ARM64):
  ...
  on the device:
    ...
    vla.sh run omnivla --mode remote --gpu           # uses rvln-omnivla-jetson
  ...
  header), e.g.:
    docker build -f docker/Dockerfile.omnivla.jetson \
      ...
      --build-arg TORCH_VERSION=2.8.0 -t rvln-omnivla-jetson .
  Force/disable Jetson mode with RVLN_JETSON=1 / =0.
...
Environment overrides:
  ...
  RVLN_JETSON  1 = force Jetson images + nvidia runtime; 0 = force x86
...
# when rvln-real isn't built. $1 = model (for the fallback warnings).
export_edge_image() {
    local model=$1
...
run_mobile_cmd_vel() {
    ...
    local launch=(
        rvln_bringup mobile_cmd_vel.launch.py
...
run_edge_local() {
    ...
    compose_add_camera "$camera_kind" "$camera_device"
    local launch=(
        rvln_bringup omnivla_edge_local.launch.py
        "device:=cuda:0"
    )
    _append_camera_launch_args launch "$camera_kind" "$camera_device"
    _append_image_topic_arg launch
