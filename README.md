...
[![ci-lint](https://github.com/NOPLAB/rvln/actions/workflows/ci-lint.yml/badge.svg?branch=main)](https://github.com/NOPLAB/rvln/actions/workflows/ci-lint.yml)
[![ci-flutter](https://github.com/NOPLAB/rvln/actions/workflows/ci-flutter.yml/badge.svg?branch=main)](https://github.com/NOPLAB/rvln/actions/workflows/ci-flutter.yml)
[![ci-web](https://github.com/NOPLAB/rvln/actions/workflows/ci-web.yml/badge.svg?branch=main)](https://github.com/NOPLAB/rvln/actions/workflows/ci-web.yml)
...
| --- | --- | --- | --- |
| `vla_edge_node` (`rvln_edge vla_edge_node`) | `/camera/image_raw` (`sensor_msgs/msg/Image`), `/rvln/goal` (`rvln_msgs/msg/GoalSpec`), `/rvln/remote_embedding` (`rvln_msgs/msg/ActionEmbedding`) | `/rvln/observation` (`rvln_msgs/msg/Observation`), `/rvln/predicted_path` (`nav_msgs/msg/Path`), `/rvln/status` (`diagnostic_msgs/msg/DiagnosticArray`), and `/rvln/embedding` (`ActionEmbedding`, when debug publishing is enabled) | Lifecycle node that combines images and goals for inference, then turns the result into a path in the robot frame. Local inference does not use Observation or remote embedding topics. |
| `vla_inference_node` (`ros2 run rvln_remote vla_inference_node`) | `/rvln/observation` (`Observation`) | `/rvln/remote_embedding` (`ActionEmbedding`) | Runs the selected backend and returns a result correlated by `frame_id`. Select it with `backend:=` in the launch file or `--backend` with `ros2 run`. |
| `path_follower_node` (`rvln_edge path_follower_node`) | `/rvln/predicted_path` (`nav_msgs/msg/Path`) | `/cmd_vel` (`geometry_msgs/msg/Twist`) | Converts a `base_link` path into velocity commands. Stops on an empty or stale path. The output topic is configurable. |
| `edge_action_grpc` (`rvln_edge edge_action_grpc_server`) | `ActionChunk` via `EdgeActionService.StreamActions` (default `0.0.0.0:50061`) | `/rvln/predicted_path` (`Path`) and `ControlAck` to the sender | Receives phone inference results and publishes an empty path if chunks stop arriving. |
| `edge_action_ws` (`rvln_edge edge_action_ws_server`) | Action chunks via WebSocket (default `0.0.0.0:8765`) | `/rvln/predicted_path` (`Path`) and an ack to the sender | Receives browser inference results and publishes an empty path if chunks stop arriving. |
...
on another host. The edge node subscribes to `/rvln/goal` with
...
`frame_id`. See [`src/rvln_msgs/`](src/rvln_msgs/) for the
...
ROS_DOMAIN_ID=42 ros2 launch rvln_remote inference.launch.py backend:=dummy

# Terminal 2: edge and follower; publish commands to a non-motor topic
ROS_DOMAIN_ID=42 ros2 launch rvln_edge edge_only.launch.py \
...
Run `ros2 launch rvln_remote inference.launch.py backend:=dummy` on the
inference PC and the `rvln_edge edge_only.launch.py` command above
...
For example, run `ros2 launch rvln_remote inference.launch.py
...
`ros2 launch rvln_bringup omnivla_edge_local.launch.py
weights_path:=/path/to/omnivla-edge.pth` and requires a CUDA-capable edge host.

To receive paths from a phone, run
`ros2 launch rvln_bringup mobile_cmd_vel.launch.py`. For a browser,
run `ros2 launch rvln_bringup phone_ws.launch.py`. Both publish to
...
| Path | Contents |
| --- | --- |
| `src/rvln_msgs/` | ROS 2 message definitions |
| `src/rvln_edge/` | Edge, path follower, and phone / browser receiver nodes |
| `src/rvln_remote/` | Inference node and backends (`dummy`, `asyncvla`, `omnivla`, `omnivla_edge`, `movla`) |
