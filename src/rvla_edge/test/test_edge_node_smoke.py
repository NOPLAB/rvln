...
from rvla_remote.backends.dummy import DummyBackend
from rvla_remote.inference_node import VLAInferenceNode
from rvla_edge.edge_node import VLAEdgeNode
...
def test_edge_node_publishes_path(ros_runtime):
    ...
    try:
        ...
        goal_pub = pub_node.create_publisher(GoalSpecMsg, '/rvla/goal', goal_qos)
        img_pub = pub_node.create_publisher(Image, '/camera/image_raw', 1)

        received_paths = []
        path_node = rclpy.create_node('test_sub')
        path_node.create_subscription(Path, '/rvla/predicted_path',
