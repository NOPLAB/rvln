"""OmniVLA-edge model architecture (Plan 2B Path 2 — model runs on the edge).

Vendored verbatim from ``external/OmniVLA/inference/model_omnivla_edge.py`` so
that ``load_state_dict(strict=True)`` against ``omnivla-edge.pth`` matches every
parameter name exactly. Do NOT refactor class/attribute names here — they are
part of the checkpoint contract. The only runtime dependency is
``efficientnet_pytorch`` (CLIP lives in the adapter, not the model).
"""
import math
import torch
