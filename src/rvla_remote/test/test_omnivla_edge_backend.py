...
def test_goal_mode_prefers_text_then_image_then_pose():
    from rvla_remote.backends.omnivla_edge import _goal_mode_for
...
@pytest.mark.skipif(
    ...
    reason='set OMNIVLA_EDGE_E2E=1 (needs CUDA + omnivla-edge.pth + CLIP + rvla_core)',
)
def test_omnivla_edge_backend_returns_scaled_action_chunk():
    import numpy as np
    import PIL.Image

    from rvla_remote.backends.omnivla_edge import OmniVLAEdgeBackend
