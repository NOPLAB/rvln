...
This is a ROS 2 Humble colcon workspace. `src/rvla_*` contains the owned ROS messages, inference core, remote ROS 2 node, edge nodes, and launch files. `proto/edge_action.proto` defines only the mobile-to-Pi contract; regenerate its stubs with `scripts/gen_proto.sh` after editing it. Edge and remote use `rvla_msgs` topics. `app/inference/` and `app/logger/` are separate Flutter apps, while `web/` is a Next.js browser client. `docker/` holds runtime images and Compose profiles; `docs/` holds operating and design notes. Python tests live in each package's `test/`, Dart tests in each app's `test/`, and web tests in `web/test/`. Downloaded weights in `models/` and runtime assets are not committed.
